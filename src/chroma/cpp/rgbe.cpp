...
Modified by Ben Steinert (2008):
 -- added methods to accept Chroma Image
...
int RGBE_ReadHeader(FILE *fp, int *width, int *height, rgbe_header_info *info) {
    ...
    if (fgets(buf, sizeof(buf) / sizeof(buf[0]), fp) == 0)
        return rgbe_error(rgbe_read_error, NULL);
    // bugfix:
    if (strcmp(buf, "\n") != 0) if (fgets(buf, sizeof(buf) / sizeof(buf[0]), fp) == 0)
        return rgbe_error(rgbe_read_error, NULL);
    //
    if (strcmp(buf, "\n") != 0)
...
bool LoadRGBE(const char *filename, Vector3 *&pixels, int &width, int &height) {
    ...
    if (f) {
        ...
        RGBE_ReadPixels_RLE(f, (float *) pixels, width, height);
        fclose(f);
        return true;
    }
    else {
...
bool SaveRGBE(const char *filename, const Vector3 *const &pixels, const int &width, const int &height) {
    ...
    RGBE_WriteHeader(f, width, height, 0);
    RGBE_WritePixels_RLE(f, (float *) pixels, width, height);
    fclose(f);
    return true;
}
